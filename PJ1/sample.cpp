...
InitLists( )
{
	...
	glNewList( BoxList, GL_COMPILE );
	    ...
	    glBegin( GL_TRIANGLE_STRIP);
            glColor3f( 1., 0., 6. );
	        glVertex3f( -1*dx,  1*dy,  1*dz );
	        glVertex3f( -1*dx,  1*dy, -1*dz);
	        glVertex3f(  0*dx,  3*dy,  0*dz);
	        glVertex3f(  1*dx,  1*dy, -1*dz);
	        glVertex3f(  1*dx,  1*dy,  1*dz);
		...
		glBegin( GL_QUADS );
			...
			glVertex3f( -dx,  dy, -dz );
//eye
			glVertex3f( -0.7*dx, 0.8*dy,  1.1*dz );
			glVertex3f( -0.2*dx, 0.8*dy,  1.1*dz );
			glVertex3f( -0.2*dx, 0.6*dy,  1.1*dz );
			glVertex3f( -0.7*dx, 0.6*dy,  1.1*dz );
		...
		glColor3f( 0.1, 0.3, 0 );
		    ...
		    glBegin( GL_LINE_LOOP );
		        for( int i = 0; i < n; i++ )
		        {
		            glVertex3f( cos(ang)/2, sin(ang)/2, 1.1 );
		            ang += dang;
		        }
		    glEnd( );
		...
		glBegin( GL_QUADS );
			...
			glColor3f( 2., 1., 0. );
			glVertex3f(  1.5*dx, 0.1*dy,  0.2*dz );
			glVertex3f(  1.5*dx, 0.1*dy, -0.2*dz );
			glVertex3f(  1.5*dx, 0.4*dy, -0.2*dz );
			glVertex3f(  1.5*dx, 0.4*dy,  0.2*dz );

			glVertex3f(  dx,     0.1*dy,  0.2*dz );
			glVertex3f(  1.5*dx, 0.1*dy,  0.2*dz );			
			glVertex3f(  1.5*dx, 0.4*dy,  0.2*dz );
			glVertex3f(  dx,     0.4*dy,  0.2*dz );
...
//left
		...
		glBegin( GL_QUADS );
			...
			glVertex3f(  1.5*dx, 0.4*dy,  0.2*dz );
			glVertex3f(  1.5*dx,-0.5*dy,  0.2*dz );
			glVertex3f(  1.2*dx,-0.5*dy,  0.2*dz );
			glVertex3f(  1.2*dx, 0.4*dy,  0.2*dz );
...
//left
		...
		glBegin( GL_QUADS );
		    ...
		    glVertex3f(  -0.3*dx, -1*dy,  0.3*dz );
			glVertex3f(  -0.3*dx, -2*dy,  0.3*dz );
			glVertex3f(  -0.6*dx, -2*dy,  0.3*dz );
			glVertex3f(  -0.6*dx, -1*dy,  0.3*dz );

		    glVertex3f(  -0.3*dx, -1*dy,  0.1*dz );
			glVertex3f(  -0.3*dx, -2*dy,  0.1*dz );
			glVertex3f(  -0.6*dx, -2*dy,  0.1*dz );
			glVertex3f(  -0.6*dx, -1*dy,  0.1*dz );
