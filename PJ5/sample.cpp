...
Animate( )
{
    ...
    TTime = (float)ms / (float)MS_PER_CYCLE;
...
Display( )
{
    ...
    if (Botton != 3) {
        ...
        if (Botton == 1 || Botton == 4) {
            Verttime = Time;
        }
        if (Botton == 1 || Botton == 2) {
            Fragtime = Time;
        }


        
        Ds = 0.1 * cos(M_PI*Fragtime);
        Dt = 0.1 * sin(M_PI*Fragtime);
        //Ds = 1.5;
        //Dt = 0.8
        ...
        // Teapot color
        ColorR = 0.52 * cos(M_PI*TTime);
        ColorG = 0.37 * sin(M_PI*TTime);
        ColorB = 0.26 * cos(M_PI*TTime);
        Shininess = 10;

        // Draw pattern
        S0 = 0.3 * cos(M_PI*Fragtime);
        T0 = 0.2 * sin(M_PI*Fragtime);
        S1 = 0.5;
        T1 = 0.4;
        Size= 2*sin(M_PI*Fragtime);
        ...
        // light color
        Pattern->SetUniformVariable( "uSpecularColor", 0.7, 0.8, 1 );
        ...
        Pattern->SetUniformVariable("uBotton", Botton);
